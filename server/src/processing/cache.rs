...
pub(super) struct File {
    ...
    /// link. This may not be modified after creating the Target, per HashSet
    /// constraints. TODO: create a public getter method to provide read-only
...
pub(super) struct Fragment {
    ...
    /// `id` and `contents` define the state of the `Fragment` that
    /// `GatherElements` depend on.
    id: String,
...
impl Cache {
    pub fn new() -> Self {
        Cache {
            files: HashSet::new(),
            targets_and_fragments: HashSet::new(),
            missing_targets_and_fragments: HashSet::new(),
            pending_files: vec![],
            root: PathBuf::new(),
        }
    }
