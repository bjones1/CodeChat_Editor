...
///
/// Item 2 also requires the cache to define what constitutes a difference
/// which would trigger outdating dependencies. The state which is checked for
/// differences is:
///
...
/// * Note that images, URLs, etc. may not work if the referring path in their new location isn't valid; these simply aren't supported.
...
pub(super) struct IdEntry {
    ...
    /// Every file whose rendered output depends on this id.
    /// Note that this does *not* capture the reverse dependency of a `Fragment`
    /// on the gather elements which list it; see `Design` and `commit_file`.
...
pub(super) enum IdState {
    ...
    /// `Fragment`s; the keys give a list of a files in which that id is defined, while the value indicates the number of definitions in that file.
...
pub(super) struct Target {
    ...
    /// The line number of this target in its file. Always 0
    /// until the pulldown-cmark HTML writer preserves line numbers. TODO: when
    /// the URL given to the Client includes an anchor, the Client must use the
    /// anchor to find the corresponding Target (the anchor is the id), then use
    /// this field to identify the line on which that id resides in order to
    /// scroll to the appropriate location in the document.
...
pub(super) struct Fragment {
    /// The code/doc block content of this element rendered as HTML. This is empty until the fragment's doc blocks are
