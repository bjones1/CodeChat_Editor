...
// ---------------
//
// <a id="EditorMode"></a>Define all possible editor modes; these are passed as a[query string](https://en.wikipedia.org/wiki/Query_string) (`http://path/to/foo.py?mode=toc`, for example) to the page's URL.
enum EditorMode {
...
//
// <a id="current_metadata"></a>This mirrors the data provided by the server -- see[SourceFileMetadata](../../server/src/webserver.rs#SourceFileMetadata).
...
const _open_lp = async (
    ...
    // Get the<code><a href="#current_metadata">current_metadata</a></code> from the provided`all_source` struct and store it as a global variable.
    ...
    // <a id="CodeChatEditor_test"></a>If tests should be run, then the[following global variable](CodeChatEditor-test.mts#CodeChatEditor_test) is function that runs them.
...
const on_save = async (only_if_dirty: boolean = false) => {
    ...
    // <a id="save"></a>Save the provided contents back to the filesystem, by sending an update
