...
// Capture events can be generated by synchronous VS Code callbacks. Serialize
// their async sends so activity, save, run, and compile rows stay in causal
// order and rejected sends are always handled.
let captureEventQueue: Promise<void> = Promise.resolve();
...
// Listener fast path. `sendCaptureEvent` deliberately checks the settings
// again after queued work starts because recording may stop while it waits.
function captureIsRecording(): boolean {
    return captureDisabledReason(loadStudySettings()) === undefined;
}
...
async function clearCaptureToken(): Promise<void> {
    const operationSnapshot = beginCaptureTokenMutation();
    // Keep deletion, Rust reconfiguration, and runtime-state changes in one
    // queue position so later refreshes cannot observe a partially-cleared
    // token state.
    await enqueueCaptureTokenOperation(async () => {
        if (!(await captureTokenOperationStillCurrent(operationSnapshot))) {
            return;
...
function queueCaptureOperation(
    description: string,
    operation: () => Promise<void>,
): void {
    captureEventQueue = appendSerializedCaptureOperation(
        captureEventQueue,
        operation,
        (err: unknown) => {
            reportCaptureFailure(
...
function queueActivityCapture(kind: ActivityKind, filePath?: string): void {
    queueCaptureOperation("activity", () => noteActivity(kind, filePath));
}

function queueCaptureEvent(
    eventType: CaptureEventType,
    filePath?: string,
    data: CaptureEventData = {},
): void {
    queueCaptureOperation(eventType, () =>
        sendCaptureEvent(eventType, filePath, data),
    );
}
...
export const activate = (context: vscode.ExtensionContext) => {
    ...
    capture_status_timer = setInterval(() => {
        runCaptureTask("capture status refresh", refreshCaptureStatus);
    }, 5000);
    ...
    context.subscriptions.push(
        vscode.workspace.onDidChangeConfiguration((event) => {
            if (event.affectsConfiguration("CodeChatEditor.Capture")) {
                runCaptureTask("capture settings reconciliation", async () => {
    ...
    runCaptureTask("capture token initialization", refreshCaptureTokenState);
    runCaptureTask("capture status initialization", refreshCaptureStatus);
                ...
                if (!subscribed) {
                    ...
                    context.subscriptions.push(
                        vscode.workspace.onDidSaveTextDocument((doc) => {
                            ...
                            queueCaptureEvent("save", doc.fileName, {
                                reason: "manual_save",
                    ...
                    context.subscriptions.push(
                        vscode.debug.onDidStartDebugSession((session) => {
                            ...
                            const active = vscode.window.activeTextEditor;
                            const filePath = active?.document.fileName;
                            queueCaptureEvent("run", filePath, {
                        ...
                        vscode.debug.onDidTerminateDebugSession((session) => {
                            ...
                            const active = vscode.window.activeTextEditor;
                            const filePath = active?.document.fileName;
                            queueCaptureEvent("run_end", filePath, {
                                sessionName: session.name,
                                sessionType: session.type,
                            });
                        }),
                    ...
                    context.subscriptions.push(
                        vscode.tasks.onDidStartTaskProcess((e) => {
                            ...
                            queueCaptureEvent("compile", filePath, {
                                taskName: task.name,
                        ...
                        vscode.tasks.onDidEndTaskProcess((e) => {
                            ...
                            const filePath = active?.document.fileName;
                            const task = e.execution.task;
                            queueCaptureEvent("compile_end", filePath, {
                                taskName: task.name,
                                taskSource: task.source,
                                exitCode: e.exitCode,
                            });
